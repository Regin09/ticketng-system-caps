import React, { useState } from "react";
import Grid from "@mui/material/Grid";
import Container from "@mui/system/Container";
import { Fragment } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";
import "./overview.css";
import axios from "axios";
import { CircularProgress } from "@mui/material";

const Overview = () => {
  const { clientCode } = useParams();
  const [ticketClient, setTicketClient] = useState([]);
  const [inProgressCount, setInProgressCount] = useState(0);
  const [doneCount, setDoneCount] = useState(0);
  const [toDoCount, setToDoCount] = useState(0);
  const [needApproval, setNeedApproval] = useState(0);
  const [delivered, setDelivered] = useState(0);

  React.useEffect(() => {
    document.title = "Overview Page";
    const clientCode = localStorage.getItem("clientCode");
    if (clientCode) {
      getTicketClientHandler(clientCode);
    }
  }, []);

  const getTicketClientHandler = async (clientCode) => {
    try {
      const res = await axios({
        method: "GET",
        url: `${process.env.REACT_APP_API_URL}/api/ticket/client/${clientCode}`,
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access_token")}`,
        },
      });
      console.log("Response GET");
      console.log(res.data.ticket);

      setTicketClient(res.data.ticket);
      const needApproval = res.data.ticket.filter(
        (data) => data.status === "Need-Approval"
      ).length;
      const delivered = res.data.ticket.filter(
        (data) => data.status === "Delivered"
      ).length;
      const inProgressCount = res.data.ticket.filter(
        (ticket) => ticket.status === "In-Progress"
      ).length;
      const doneCount = res.data.ticket.filter(
        (ticket) => ticket.status === "Done"
      ).length;
      const toDoCount = res.data.ticket.filter(
        (ticket) => ticket.status === "To-Do"
      ).length;

      setInProgressCount(inProgressCount);
      setDoneCount(doneCount);
      setToDoCount(toDoCount);
      setDelivered(delivered);
      setNeedApproval(needApproval);
    } catch (error) {
      if (error.response.status === 404) {
      }
      console.log(error);
    }
  };
  return (
    <Container>
      <Grid container spacing={5}>
        <Grid item md={4} xl={4} sm={4} className="centerGrid">
          <div className="card-example">
            <h1 className="h1-pad">Need Approval </h1>
            <h3 style={{ color: "#4C4B16" }}>{needApproval}</h3>
          </div>
        </Grid>
        <Grid item md={4} xl={4} sm={4} className="centerGrid">
          <div className="card-example">
            <h1 className="h1-pad">In-Progress Tickets </h1>
            <h3 style={{ color: "#1B8500" }}>{inProgressCount}</h3>
          </div>
        </Grid>
        <Grid item md={4} xl={4} sm={4} className="centerGrid">
          <div className="card-example">
            <h1 className="h1-pad">Delivered </h1>
            <h3 style={{ color: "##1D5D9B" }}>{delivered}</h3>
          </div>
        </Grid>
        <Grid item md={6} xl={6} sm={6} className="centerGrid">
          <div className="card-example">
            <h1 className="h1-pad">Done Tickets</h1>
            <h3 style={{ color: "red" }}>{doneCount}</h3>
          </div>
        </Grid>
        <Grid item md={6} xl={6} sm={6} className="centerGrid">
          <div className="card-example">
            <h1 className="h1-pad">To-Do Tickets</h1>
            <h3 style={{ color: "#FF8A00" }}>{toDoCount}</h3>
          </div>
        </Grid>
      </Grid>
    </Container>
  );
};
export default Overview;
